 ...
 * April 2016
 * 
 * Purpose: Make an audible alarm/tune on request from a client.
 * For example, some WiFi security webcams have a feature that will access a
 * specified URL when motion is detected by the camera. Alternatively,
 * a WiFi enabled barometric pressure sensor could request a "storm approaching" alarm
 * if the pressure drops rapidly.
 * 
 * Credits to:
 * Brett Hagman for RTTTL code
 * Mat Hughson (tweedius) for use of analogWriteFreq() to replace Tone()
 * And whoever wrote the WiFiWebServer example!
 * 
 * To Do:
 * Amend to use tone() instead of analogWriteFreq(). tone() function now reported 
 * working for esp8266.
 * Amend to allow different tunes to be requested by clients.
 * 
 */
...
char *song = "GoodBad:d=4,o=5,b=56:32p,32a#,32d#6,32a#,32d#6,8a#.,16f#.,16g#.,d#";
...
void play_rtttl(char *p)
{
  ...
  while(*p)
  {
    ...
    if(note)
    {
      ...
      analogWriteFreq(notes[(scale - 4) * 12 + note]);
      analogWrite(buzPin, 500);
      delay(duration);
